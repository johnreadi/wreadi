...
interface PiecesConfiguratorProps {
    initialProducts: Product[];
}

export function PiecesConfigurator({ initialProducts }: PiecesConfiguratorProps) {
    ...
    useEffect(() => {
        // Extract unique brands, series, models from initial products
        const allBrands = initialProducts.map((p) => p.brand).filter((b): b is string => b !== null);
        setBrands(Array.from(new Set(allBrands)));

        const allSeries = initialProducts.map((p) => p.series).filter((s): s is string => s !== null);
        setSeries(Array.from(new Set(allSeries)));

        const allModels = initialProducts.map((p) => p.model).filter((m): m is string => m !== null);
    ...
    return (
        <>
            ...
            <section id="configurateur" className="py-12 bg-gray-50 border-t">
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select value={selectedSeries || "all"} onValueChange={(val) => setSelectedSeries(val === "all" ? "" : val)} disabled={!selectedBrand}>
                                        <SelectTrigger>
                                ...
                                <div>
                                    ...
                                    <Select value={selectedModel || "all"} onValueChange={(val) => setSelectedModel(val === "all" ? "" : val)} disabled={!selectedSeries}>
                                        <SelectTrigger>
